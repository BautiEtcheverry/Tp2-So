...
static int cmd_nice(int argc, char *argv[]) {
    if (argc < 3) { printf("Usage: nice <pid> <priority>\n"); return 1; }
...
/* Proceso que corre indefinidamente con espera activa */
...
commands[] = {
    ...
    { "nice",        cmd_nice,     "Change priority: nice <pid> <priority>" },
    { "block",       cmd_block,    "Block a process: block <pid>" },
    { "unblock",     cmd_unblock,  "Unblock a process: unblock <pid>" },
    { "loop",        cmd_loop,     "Spawn an endless_loop process" },
    { NULL,          NULL,         NULL }
};  
